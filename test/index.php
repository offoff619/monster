...
data-target="#exampleModal"  >เพิ่มเจ้าหน้าที่</button>
<br><br>
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
      ...
      <div class="modal-body">
        <form id="form_user">
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="a-username" name="username" autocomplete="off">
          </div>
          <div class="form-group">
            <label for="user-pass" class="control-label">Password:</label>
            <input type="password" class="form-control" id="a-password" name="password" autocomplete="off">
          </div>
          <div class="form-group">
            <label for="user-fullname" class="control-label">Name:</label>
            <input type="text" class="form-control" id="a-name" name="fullname" autocomplete="off">
          </div>
		  <div class="form-group">
            <label for="user-lastname" class="control-label">Lastname:</label>
            <input type="text" class="form-control" id="a-lasttname" name="username" autocomplete="off">
          </div>
		  <div class="form-group">
            <label for="user-position" class="control-label">Position:</label>
            <select class="form-control"  id="a-position" name="position">
  ...
  <textarea class="form-control" rows="5" id="a-address" name="Address"></textarea>
		</div>
		<div class="form-group">
            <label for="user-name" class="control-label">Level:</label>
            <select class="form-control"  id="a-level" name="userposition">
              <option value="1">Type 1</option>
              <option value="2">Type 2</option>
            </select> 
			<input type="hidden" id="user-id" name="userid" value="">
		...
		if(chk_user_id!=null){
			...
			dataList.getItem(chk_user_id).done(function(res){
				if(res != null && res.data.length > 0){
					...
					modal.find("#a-username").val(res.data[0].mem_user);
					modal.find("#a-password").val(res.data[0].mem_pass);
					modal.find("#a-firstname").val(res.data[0].mem_firstname);
					modal.find("#a-lasttname").val(res.data[0].mem_lasttname);
					modal.find("#a-position").val(res.data[0].mem_position);
					modal.find("#a-address").val(res.data[0].mem_address);
					modal.find("#a-level").val(res.data[0].mem_level);
